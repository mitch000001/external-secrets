...
func (r *Reconciler) getProviderSecretData(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, crClient client.Client) (map[string][]byte, error) {
	...
	for i, remoteRef := range externalSecret.Spec.DataFrom {
		...
		} else if remoteRef.Generator != nil {
			secretMap, err = r.handleGenerateSecrets(ctx, externalSecret, remoteRef, i)
		}
...
func (r *Reconciler) handleGenerateSecrets(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, remoteRef esv1beta1.ExternalSecretDataFromRemoteRef, i int) (map[string][]byte, error) {
	gen, err := genv1alpha1.GetGenerator(remoteRef.Generator)
	if err != nil {
		return nil, err
	}
	secretMap, err := gen.Generate(ctx, remoteRef.Generator, r.Client, externalSecret.Namespace)
	if err != nil {
...
func (r *Reconciler) handleExtractSecrets(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, remoteRef esv1beta1.ExternalSecretDataFromRemoteRef, i int) (map[string][]byte, error) {
	client, err := r.getClientOrDefault(ctx, externalSecret.Spec.SecretStoreRef, externalSecret.Namespace, remoteRef.SoureRef)
	if err != nil {
		return nil, err
	}
	defer client.Close(ctx)
